...
module -q load cmake
module -q load rocm
#LYRA module -q load openmpi
#LYRA MY_MPI_ROOT=OLCF_OPENMPI_ROOT
MY_MPI_ROOT=$CRAY_MPICH_DIR
...
cmake \
 ...
 \
  ...
  -DCMAKE_CXX_FLAGS:STRING="-DNM_VALUE=$NM_VALUE $HIPCC_FLAGS -I$MY_MPI_ROOT/include -DUSE_MPI" \
 \
  -DCMAKE_EXE_LINKER_FLAGS:STRING="-L$MY_MPI_ROOT/lib -Wl,-rpath,$MY_MPI_ROOT/lib -lmpi" \
 \
  -DUSE_HIP:BOOL=ON \
 \
  $SOURCE
