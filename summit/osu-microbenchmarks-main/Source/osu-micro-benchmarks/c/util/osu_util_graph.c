...
int omb_graph_init(omb_graph_options_t *graph_options)
{
    ...
    OMB_CHECK_NULL_AND_EXIT(graph_options->gnuplot_pointer, "Unable to open"
            " Gnuplot.\nPlease check Gnuplot installation\n");
    if (options.graph_output_png || options.graph_output_pdf) {
        sprintf(cmd, "%s -e 'print GPVAL_TERMINALS' 2>&1 | tr ' ' '\n'"
                " |grep png > /dev/null", OMB_GNUPLOT_PATH);
        return_code = system(cmd);
        if (1 == WEXITSTATUS(return_code)) {
            fprintf(stderr, "Error: Gnuplot does not support png"
        ...
        size_t message_size, size_t length)
{
    ...
    if (0 == graph_options->number_of_graphs) {
        graph_options->graph_datas = malloc(sizeof(struct graph_data*) *
                ((int)log2(options.max_message_size) + 2));

    }
    graph_options->graph_datas[graph_options->number_of_graphs++] =
        graph_data;
    return;
}
...
void omb_graph_plot(omb_graph_options_t *graph_options, const char *filename)
{
    ...
    for(graph_itr = 0; graph_itr < graph_options->number_of_graphs; graph_itr++)
    {
        ...
        fprintf(graph_options->gnuplot_pointer, "set xlabel \"Iteration"
                " Count\"\n");
        fprintf(graph_options->gnuplot_pointer, "set title \"Size: %zu, Avg:"
                "%.*f\"\n", graph_data->message_size, FLOAT_PRECISION,
                graph_data->avg);
        if (options.graph_output_term) {
            fprintf(graph_options->gnuplot_pointer, "set terminal dumb %zu %zu\n",
                    terminal_size.col_size, terminal_size.row_size);
        ...
        if (options.graph_output_png || options.graph_output_pdf) {
            sprintf(png_file_name, "%s-%zu", filename, graph_data->message_size);
            fprintf(graph_options->gnuplot_pointer, "set terminal png size %d,"
            ...
            fprintf(graph_options->gnuplot_pointer, "plot '-' with linespoints"
    ...
    if (options.graph_output_png || options.graph_output_pdf) {
        ...
        for (l = 0; l < graph_options->number_of_graphs; l++) {
            ...
            for(k = 0; k < graph_data->length; k++) {
                fprintf(graph_options->gnuplot_pointer, "%d %d %.*lf\n", l, k + 1,
                                OMB_GRAPH_FLOATING_PRECISION, graph_data->data[k]);
            }
        ...
        for (i = 0; i < OMB_3D_GRAPH_PARTS; i++) {
            ...
            fprintf(graph_options->gnuplot_pointer, "set title \"%s\"\n", (i ==
                        0) ? "Small Message Size" : "Large Message Size");
            if (options.bench == MBW_MR ||
                    options.subtype == BW) {
                fprintf(graph_options->gnuplot_pointer, "set zlabel rotate"
            ...
            fprintf(graph_options->gnuplot_pointer, "set xlabel rotate"
                    " parallel offset 0,-3 \"{/:Bold Size (Bytes)}\" font"
                    " \",%d\"\n", OMB_GNUPLOT_PNG_FONT_SIZE);
            fprintf(graph_options->gnuplot_pointer, "set ylabel rotate"
            ...
            fprintf(graph_options->gnuplot_pointer, "set arrow from graph 1,0,0"
                    " to graph 1.1,0,0 filled\n");
            fprintf(graph_options->gnuplot_pointer, "set arrow from graph"
                    " 0,0,0.1 to graph 0,0,1.1 filled\n");
            fprintf(graph_options->gnuplot_pointer, "set arrow from graph 1,0,0"
                    " to graph 1,1.1,0 filled\n");
            fprintf(graph_options->gnuplot_pointer, "set ytics offset 0,-1 font"
                    " \",%d\"\n", OMB_GNUPLOT_PNG_FONT_SIZE);
            fprintf(graph_options->gnuplot_pointer, "set ztics font \",%d\"\n",
                    OMB_GNUPLOT_PNG_FONT_SIZE);
            fprintf(graph_options->gnuplot_pointer, "set title font \",%d\"\n",
                    OMB_GNUPLOT_PNG_FONT_SIZE);
            fprintf(graph_options->gnuplot_pointer, "set xtics offset 0,-2 font"
                    " \",%d\" (", OMB_GNUPLOT_PNG_FONT_SIZE);
            ...
            while (current_graph_index <= graph_options->number_of_graphs) {
                fprintf(graph_options->gnuplot_pointer, "'%s' using 1:2:3 index"
                        " %d with linespoints pt 20 lc  %d t sprintf('%zu') ",
                        OMB_GNUPLOT_DATA_FILENAME, current_graph_index,
                        gnuplot_index,
                        graph_options->graph_datas[current_graph_index]->message_size);
                gnuplot_index++;
...
void omb_graph_allocate_and_get_data_buffer(omb_graph_data_t **graph_data,
        omb_graph_options_t *graph_options, size_t message_size, size_t length)
{
    int rank = -1;
    ...
    if (options.graph && 0 == rank) {
        ...
        *graph_data = graph_options->graph_datas[
            graph_options->number_of_graphs - 1];
    }
}
